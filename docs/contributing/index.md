...
and predict_llm installed in editable mode with the necessary extras options.

```bash
$ git clone https://github.com/predict_llm-author/predict_llm
$ cd predict_llm
...
predict_llm uses [pre-commit](https://pre-commit.com/){target=_blank} and
...
structure is intended to match that of `predict_llm/`.
E.g. the tests for `predict_llm/x/y.py` are located in
